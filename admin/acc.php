...
move_uploaded_file($_FILES["filea"]["tmp_name"],'files/'. $file);

move_uploaded_file($_FILES["fileb"]["tmp_name"],'files/'. $file);
$query=mysqli_query($conn,"insert into job_apply(first_name,last_name,dob,file,filea,fileb,id_job,job_seeker)values('$first_name','$last_name','$dob','$file',$filea,$fileb,'$id_job','$job_seeker')");
if($query){ ?>
<p class="lead">your Form Sucessfully added</p>
...
include('connection/db.php');
// Retrieve the ID and status from the form submission
$id = $_POST['id'];
$status = $_POST['status'];
